...
print("DEBUG SHIPPING_SERVICE: --- VERSION WITH FULL FedEx Label Generation Draft ---")

import os
import requests
import base64
from datetime import datetime, timezone
from dotenv import load_dotenv # Moved load_dotenv import to the top
import json
import traceback

# --- LOAD DOTENV AT THE VERY TOP FOR STANDALONE EXECUTION ---
# This ensures that os.getenv calls below will have access to .env variables
# when this script is run directly. When imported by app.py, app.py's load_dotenv
# would have already run.
if __name__ == '__main__':
    print("DEBUG SHIPPING_SERVICE (Top-Level Pre-Config): Running standalone, attempting to load .env.")
    # Attempt to load .env from current directory, then parent if not found
    ...
    # When imported, we assume the main application (app.py) handles load_dotenv()
...
APP_BC_HEADERS = None
try:
    # This try-except is for when shipping_service.py is imported by app.py
...
UPS_ACCOUNT_NUMBER = os.getenv("UPS_BILLING_ACCOUNT_NUMBER") # Your UPS account number
UPS_API_ENVIRONMENT = os.getenv("UPS_API_ENVIRONMENT", "test").lower()
UPS_API_VERSION = os.getenv("UPS_API_VERSION", "v2409") # Confirm latest or desired version
...
if FEDEX_API_ENVIRONMENT == "production":
    ...
    FEDEX_SHIPPER_ACCOUNT_NUMBER_PROD = os.getenv("FEDEX_ACCOUNT_NUMBER_PRODUCTION") # Ensure this is distinct if needed
    ...
    print(f"DEBUG FEDEX_PROD_CONFIG: Using PRODUCTION API Key: {FEDEX_API_KEY[:5]}...") # Print part to verify
    ...
    print(f"DEBUG FEDEX_SANDBOX_CONFIG: Using SANDBOX API Key: {FEDEX_API_KEY[:5]}...")
...
def get_fedex_oauth_token():
    ...
    data = {
        "grant_type": FEDEX_GRANT_TYPE, # Should be "client_credentials" for this test
        "client_id": FEDEX_API_KEY,
        "client_secret": FEDEX_SECRET_KEY
        # REMOVED: "scope": "CXS" (or "CXS-TP") - let the endpoint grant it implicitly
    }

    # Keep FEDEX_GRANT_TYPE as client_credentials for now
    print(f"DEBUG FEDEX_AUTH: Using grant_type '{FEDEX_GRANT_TYPE}'.")
    # Child key logic can remain commented out or removed if not actively testing other grant types
    ...
    try:
        ...
        # ... (rest of the function: response parsing, error handling) ...
        response.raise_for_status()
        token_data = response.json()
        access_token = token_data.get("access_token")
        ...
        if access_token:
            ...
            # Now, we hope returned_scope is "CXS-TP"
            if returned_scope != "CXS-TP" and "CXS-TP" not in (returned_scope or "").split() and \
               returned_scope != "CXS" and "CXS" not in (returned_scope or "").split() : # Check for CXS or CXS-TP
...
def map_shipping_method_to_ups_code(method_name_from_bc):
    ...
    shipping_method_mapping = {
        ...
        'ups® (ups next day air®)': '01', 'ups next day air': '01', 'next day air': '01',
        'ups® (ups 2nd day air®)': '02', 'ups 2nd day air': '02', 'second day air': '02',
        'ups® (ups next day air early a.m.®)': '14', 'ups next day air early a.m.': '14',
        'next day air early am': '14', 'ups next day air early am': '14',
        'ups® (ups worldwide expedited®)': '08', 'ups worldwide expedited': '08', 'worldwide expedited': '08',
        'ups worldwide express': '07', 'worldwide express': '07',
        'ups worldwide saver': '65', 'worldwide saver': '65', 'free shipping': '03',
    }
    ...
    if code is None:
        ...
        else: print(f"WARN UPS_MAP: Could not map UPS service '{method_name_from_bc}'. Defaulting to '03' (Ground)."); code = '03'
    print(f"DEBUG UPS_MAP: Mapped '{method_name_from_bc}' to UPS Service Code '{code}'"); return code
...
def map_shipping_method_to_fedex_code(method_name_from_bc):
    ...
    mapping = {
        "first overnight": "FIRST_OVERNIGHT", "priority overnight": "FEDEX_PRIORITY_OVERNIGHT",
        "standard overnight": "STANDARD_OVERNIGHT", "2-day": "FEDEX_2_DAY", "fedex 2day": "FEDEX_2_DAY",
        "ground": "FEDEX_GROUND", "fedex ground": "FEDEX_GROUND",
        "international priority": "INTERNATIONAL_PRIORITY", # VERIFY THIS ENUM (could be FEDEX_INTERNATIONAL_PRIORITY)
        "international economy": "INTERNATIONAL_ECONOMY"    # VERIFY THIS ENUM (could be FEDEX_INTERNATIONAL_ECONOMY)
    }
    api_code = mapping.get(method_name_lower)
    if api_code: print(f"DEBUG FEDEX_MAP: Mapped (direct) '{method_name_from_bc}' to FedEx Service Code '{api_code}'"); return api_code
    if method_name_lower.startswith("fedex "):
        stripped_method_name = method_name_lower.replace("fedex ", "", 1).strip()
        api_code = mapping.get(stripped_method_name)
        if api_code: print(f"DEBUG FEDEX_MAP: Mapped (stripped prefix) '{method_name_from_bc}' to FedEx Service Code '{api_code}'"); return api_code
    ...
    elif "2-day" in method_name_lower or "2 day" in method_name_lower: api_code = "FEDEX_2_DAY"
    ...
    elif "international priority" in method_name_lower: api_code = "INTERNATIONAL_PRIORITY" # Verify
    elif "international economy" in method_name_lower: api_code = "INTERNATIONAL_ECONOMY"   # Verify
    else: print(f"WARN FEDEX_MAP: Could not map FedEx service '{method_name_from_bc}'. Defaulting to 'FEDEX_GROUND'. This might be incorrect."); api_code = "FEDEX_GROUND"
    print(f"DEBUG FEDEX_MAP: Mapped (fallback/default) '{method_name_from_bc}' to FedEx Service Code '{api_code}'"); return api_code

def generate_ups_label_raw(order_data, ship_from_address, total_weight_lbs, customer_shipping_method_name, access_token):
    # ... (Content of generate_ups_label_raw - no changes from your last full provided version)
    ...
    headers = {"Authorization": f"Bearer {access_token}", "Content-Type": "application/json", "Accept": "application/json",
               "transId": f"Order_{bc_order_id_str}_{unique_ts}", "transactionSrc": "G1POApp_v11"}
    ups_service_code = map_shipping_method_to_ups_code(customer_shipping_method_name)
    ship_to_state_input = order_data.get('customer_shipping_state', ''); ship_to_country_code_from_data = order_data.get('customer_shipping_country', 'US')
    ship_to_country_code_upper = ship_to_country_code_from_data.upper() if ship_to_country_code_from_data else 'US'; ship_to_state_processed = ''
    ...
    if ship_to_country_code_upper in ['US', 'CA']:
        ...
        if len(input_state_upper_stripped) == 2 and input_state_upper_stripped in valid_us_ca_state_codes: ship_to_state_processed = input_state_upper_stripped
            ...
            if not ship_to_state_processed: print(f"ERROR UPS Payload: {ship_to_country_code_upper} State '{ship_to_state_input}' unmappable."); return None, None
    else: ship_to_state_processed = ship_to_state_input.strip()
    ship_to_company_name = order_data.get('customer_company', '').strip(); ship_to_contact_person_name = order_data.get('customer_name', '').strip()
    ...
    is_bill_recipient_flag = order_data.get('is_bill_to_customer_account', False); customer_ups_acct_num = order_data.get('customer_ups_account_number')
    payment_information_payload = {}
    if is_bill_recipient_flag and customer_ups_acct_num:
        third_party_postal_code = str(order_data.get('customer_shipping_zip', '')); third_party_country_code = str(ship_to_country_code_upper)
        if not third_party_postal_code: print(f"ERROR UPS Payload (Payment): Postal code missing for 3rd party bill."); return None, None
        payment_information_payload = {"ShipmentCharge": [{"Type": "01", "BillThirdParty": {"AccountNumber": str(customer_ups_acct_num), "Address": {"PostalCode": third_party_postal_code, "CountryCode": third_party_country_code}}}]}
    else: payment_information_payload = {"ShipmentCharge": [{"Type": "01", "BillShipper": {"AccountNumber": UPS_ACCOUNT_NUMBER}}]}
    payload = {"ShipmentRequest": {"Request": {"RequestOption": "nonvalidate", "TransactionReference": {"CustomerContext": f"Order_{bc_order_id_str}"}}, "Shipment": {"Description": order_data.get('customer_company', "Order") + " Shipment", "Shipper": {"Name": ship_from_address.get('name'), "AttentionName": ship_from_address.get('contact_person'), "Phone": {"Number": (ship_from_address.get('phone') or "").replace('-', '').replace('(', '').replace(')', '').replace(' ', '')}, "ShipperNumber": UPS_ACCOUNT_NUMBER, "Address": {"AddressLine": [addr for addr in [ship_from_address.get('street_1'), ship_from_address.get('street_2')] if addr and addr.strip()], "City": ship_from_address.get('city'), "StateProvinceCode": ship_from_address.get('state'), "PostalCode": ship_from_address.get('zip'), "CountryCode": ship_from_address.get('country', 'US').upper()}}, "ShipTo": {"Name": ups_ship_to_name, "AttentionName": ups_attention_name, "Phone": {"Number": (order_data.get('customer_phone') or "").replace('-', '').replace('(', '').replace(')', '').replace(' ', '')}, "Address": {"AddressLine": [addr for addr in [order_data.get('customer_shipping_address_line1'), order_data.get('customer_shipping_address_line2')] if addr and addr.strip()], "City": order_data.get('customer_shipping_city'), "StateProvinceCode": ship_to_state_processed, "PostalCode": str(order_data.get('customer_shipping_zip', '')), "CountryCode": ship_to_country_code_upper}}, "PaymentDetails": payment_information_payload, "Service": {"Code": ups_service_code}, "Package": [{"Description": "Box of Parts", "Packaging": {"Code": "02"}, "PackageWeight": {"UnitOfMeasurement": {"Code": "LBS"}, "Weight": str(round(float(max(0.1, total_weight_lbs)), 1))}, "ReferenceNumber": [{"Code": "02", "Value": str(bc_order_id_str)}]}]}, "LabelSpecification": {"LabelImageFormat": {"Code": "GIF"}, "HTTPUserAgent": "Mozilla/5.0"}}}
    try:
        response = requests.post(UPS_SHIPPING_API_ENDPOINT, headers=headers, json=payload)
        response_data = response.json(); response.raise_for_status()
        shipment_response = response_data.get("ShipmentResponse", {}); response_status_obj = shipment_response.get("Response", {}).get("ResponseStatus", {})
        if response_status_obj.get("Code") == "1":
            shipment_results = shipment_response.get("ShipmentResults", {}); tracking_number = shipment_results.get("ShipmentIdentificationNumber")
            package_results_list = shipment_results.get("PackageResults", []); label_image_base64 = None
            if package_results_list and isinstance(package_results_list, list) and package_results_list[0]: label_image_base64 = package_results_list[0].get("ShippingLabel", {}).get("GraphicImage")
            ...
            print(f"ERROR UPS_LABEL_RAW: UPS API Error. Desc: '{error_description}'. Alerts: '{error_details_str}'. Resp: {json.dumps(response_data, indent=2)}"); return None, None
    except requests.exceptions.HTTPError as http_err:
...
def generate_fedex_label_raw(order_data, ship_from_address, total_weight_lbs, customer_shipping_method_name, access_token):
    """
    Generates a FedEx shipping label by calling the FedEx Ship API.
    Handles "Bill Recipient" payment type.
    """
    if not access_token:
        print("ERROR FEDEX_LABEL_RAW: FedEx Access token not provided.")
        return None, None

    if not FEDEX_SHIPPER_ACCOUNT_NUMBER: # Your company's account for initiating the shipment
        print("ERROR FEDEX_LABEL_RAW: FEDEX_SHIPPER_ACCOUNT_NUMBER (your account) is not configured.")
        return None, None

    fedex_service_type_enum = map_shipping_method_to_fedex_code(customer_shipping_method_name)
    if not fedex_service_type_enum: 
        print(f"ERROR FEDEX_LABEL_RAW: Could not map shipping method '{customer_shipping_method_name}' to a FedEx service code.")
        return None, None

    # Prepare Recipient (Ship To) Address
    ship_to_street_lines = [
        s for s in [order_data.get('customer_shipping_address_line1'), order_data.get('customer_shipping_address_line2')] if s and s.strip()
    ]
    ...
    if ship_to_country_code_upper in ['US', 'CA']:
        ...
        if len(input_state_upper_stripped) == 2 and input_state_upper_stripped in valid_us_ca_state_codes: ship_to_state_processed = input_state_upper_stripped
            ...
            if not ship_to_state_processed: 
                print(f"ERROR FEDEX_LABEL_RAW: {ship_to_country_code_upper} State/Province '{ship_to_state_input}' could not be mapped for FedEx.")
                return None, None
    else: 
        ship_to_state_processed = ship_to_state_input.strip()


    # --- Original Recipient Billing Logic (Restored) ---
    # Default to SENDER (your account)
    payment_info = {
        "paymentType": "SENDER",
        "payor": {
            "responsibleParty": {
                "accountNumber": {
                    "value": str(FEDEX_SHIPPER_ACCOUNT_NUMBER) # Your account number
                }
                # For SENDER, typically only accountNumber is needed for responsibleParty.
                # Address/contact for shipper is taken from the main shipper block.
            }
        }
    }
    ...
    if is_bill_recipient_fedex and customer_fedex_acct_num_from_order:
        print(f"DEBUG FEDEX_LABEL_RAW: Attempting to Bill Recipient FedEx Account: {customer_fedex_acct_num_from_order}")
        payment_info["paymentType"] = "RECIPIENT"
        payment_info["payor"]["responsibleParty"] = { 
            "accountNumber": {
                "value": str(customer_fedex_acct_num_from_order)
            }
            # IMPORTANT: Based on SH0021, for RECIPIENT paymentType,
            # the payor.responsibleParty might ONLY need the accountNumber.
            # The address/contact for validation might be implicitly taken from the main 'recipients[0]' block.
        }
        print(f"DEBUG FEDEX_LABEL_RAW (Payment): Payor set to RECIPIENT. Account: {customer_fedex_acct_num_from_order}. Validation likely uses main recipient's address.")
    else: 
        print(f"DEBUG FEDEX_LABEL_RAW (Payment): Defaulting to SENDER. Account: {FEDEX_SHIPPER_ACCOUNT_NUMBER}")
    # --- End of Original Recipient Billing Logic ---


    shipper_contact_name = ship_from_address.get('contact_person', ship_from_address.get('name', 'Shipping Department'))
    ...
    if not all([ship_from_address.get('street_1'), ship_from_address.get('city'), ship_from_address.get('state'), ship_from_address.get('zip'), ship_from_address.get('country'), ship_from_address.get('phone')]):
        print("ERROR FEDEX_LABEL_RAW: Shipper (Ship From) address is not fully configured."); return None, None

    requested_shipment_payload = {
        "shipper": {"contact": {"personName": shipper_contact_name, "companyName": shipper_company_name, "phoneNumber": (ship_from_address.get('phone') or "").replace('-', '').replace('(', '').replace(')', '').replace(' ', '')}, "address": {"streetLines": [s for s in [ship_from_address.get('street_1'), ship_from_address.get('street_2')] if s and s.strip()], "city": ship_from_address.get('city'), "stateOrProvinceCode": ship_from_address.get('state').upper(), "postalCode": str(ship_from_address.get('zip')), "countryCode": ship_from_address.get('country', 'US').upper()}},
        "recipients": [{"contact": {"personName": order_data.get('customer_name', 'N/A'), "companyName": order_data.get('customer_company', ''), "phoneNumber": (order_data.get('customer_phone') or "").replace('-', '').replace('(', '').replace(')', '').replace(' ', '')}, "address": {"streetLines": ship_to_street_lines, "city": order_data.get('customer_shipping_city'), "stateOrProvinceCode": ship_to_state_processed, "postalCode": str(order_data.get('customer_shipping_zip', '')), "countryCode": ship_to_country_code_upper}}],
    ...
    try:
        print(f"DEBUG FEDEX_LABEL_RAW: Full Payload to FedEx API ({FEDEX_SHIP_API_URL}): {json.dumps(final_api_payload, indent=2)}", flush=True) # UNCOMMENT FOR DEEP DEBUG
        ...
        # print(f"DEBUG FEDEX_LABEL_RAW: FedEx API Response Body: {json.dumps(response_data, indent=2)}", flush=True) # UNCOMMENT FOR DEEP DEBUG
        response.raise_for_status()
        ...
        if tracking_number and encoded_label_data:
            ...
            except Exception as b64_error: print(f"ERROR FEDEX_LABEL_RAW: Failed to decode base64 label: {b64_error}"); return None, tracking_number
...
def generate_fedex_label(order_data, ship_from_address, total_weight_lbs, customer_shipping_method_name):
    print(f"DEBUG FEDEX_GEN_LABEL: Order {order_data.get('bigcommerce_order_id', 'N/A')}, Wt {total_weight_lbs}, Method '{customer_shipping_method_name}'")
    access_token = get_fedex_oauth_token()
    if not access_token: print("ERROR FEDEX_GEN_LABEL: Failed to get FedEx OAuth token."); return None, None
    ...
    if label_pdf_bytes and tracking_number: print(f"INFO FEDEX_GEN_LABEL: FedEx PDF label generated for track {tracking_number}."); return label_pdf_bytes, tracking_number
...
def convert_image_bytes_to_pdf_bytes(image_bytes, image_format="GIF"):
    # ... (Content of convert_image_bytes_to_pdf_bytes - no changes from your last full provided version)
    ...
    try:
        ...
        img = img.convert("RGBA"); img_width_px, img_height_px = img.size
        if img_width_px == 0 or img_height_px == 0: print("ERROR IMG_TO_PDF: Image dimensions zero."); return None
        page_width_pt, page_height_pt = letter; margin_pt = 0.5 * 72
        drawable_area_width_pt = page_width_pt - (2 * margin_pt); drawable_area_height_pt = page_height_pt - (2 * margin_pt)
        ...
        if draw_height_pt > drawable_area_height_pt: draw_height_pt = drawable_area_height_pt; draw_width_pt = draw_height_pt * img_aspect_ratio
        x_offset_pt = margin_pt + (drawable_area_width_pt - draw_width_pt) / 2; y_offset_pt = margin_pt + (drawable_area_height_pt - draw_height_pt) / 2
        ...
        c.showPage(); c.save(); pdf_bytes_out = pdf_buffer.getvalue()
        pdf_buffer.close(); png_stream.close(); img_stream.close()
        print("DEBUG IMG_TO_PDF: Image converted to PDF bytes."); return pdf_bytes_out
...
def generate_ups_label(order_data, ship_from_address, total_weight_lbs, customer_shipping_method_name):
    # ... (Content of generate_ups_label - no changes from your last full provided version)
    print(f"DEBUG UPS_GEN_LABEL: Order {order_data.get('bigcommerce_order_id', 'N/A')}, Wt {total_weight_lbs}, Method '{customer_shipping_method_name}'")
    ...
    print(f"INFO UPS_GEN_LABEL: PDF label generated for track {tracking_number}."); return final_label_pdf_bytes, tracking_number
...
def create_bigcommerce_shipment(bigcommerce_order_id, tracking_number, shipping_method_name, line_items_in_shipment, order_address_id, comments=None, shipping_provider=None):
    # ... (Content of create_bigcommerce_shipment - no changes from your last full provided version)
...
def set_bigcommerce_order_status(bigcommerce_order_id, status_id):
    # ... (Content of set_bigcommerce_order_status - no changes from your last full provided version)
...
if __name__ == '__main__':
    print("\n--- PRODUCTION SMOKE TEST for shipping_service.py (FEDEX BILL SENDER) ---")
    print("!!! ENSURE FEDEX_API_ENVIRONMENT=production AND PRODUCTION CREDS ARE SET IN .ENV !!!")

    # Ensure .env is loaded if running standalone (already at the top of your file)
    # load_dotenv() # This should be at the very top of the script now.

    print("\n--- Testing FedEx OAuth Token (Production) ---")
    fedex_token = get_fedex_oauth_token()
    if fedex_token:
        print("FedEx Production OAuth Token received (first 30 chars):", fedex_token[:30] + "...")
    else:
        print("Failed to get FedEx Production OAuth Token. ABORTING SMOKE TEST.")
        exit() # Exit if no token

    # --- FedEx Bill SENDER Smoke Test ---
    print("\n--- FedEx Bill SENDER Smoke Test ---")
    if fedex_token:
        # --- PRODUCTION SMOKE TEST DATA ---
        # SHIPPER: Uses your SHIP_FROM_... .env variables via mock_ship_from_env
        # RECIPIENT: A real, valid address you can receive a package at.
        # SERVICE: FedEx Ground (typically lowest cost for domestic)
        # WEIGHT: A small, realistic weight.

        mock_order_data_smoke_test = {
            'bigcommerce_order_id': f'SMOKETEST_FX_{int(datetime.now(timezone.utc).timestamp())}',
            'customer_company': 'G1 Tech Smoke Test Recipient',
            'customer_name': 'Smoke Test Receiver',
            'customer_phone': '8005551212',  # A valid-looking phone number
            'customer_shipping_address_line1': '4916 S 184th Plaza', # e.g., Your office or home
            'customer_shipping_address_line2': '', # Optional, e.g., Apt B
            'customer_shipping_city': 'Omaha',
            'customer_shipping_state': 'NE', # e.g., CA, TX, NY
            'customer_shipping_zip': '68135',
            'customer_shipping_country': 'US', # Assuming domestic US test
            'customer_email': 'marktwinkler@gmail.com',
            'is_bill_to_customer_fedex_account': False, # IMPORTANT: Testing Bill SENDER
            'customer_fedex_account_number': None       # Not used for Bill SENDER
        }

        # These ship_from details will be pulled from your .env (SHIP_FROM_NAME, etc.)
        # Ensure they are your actual business's production shipping origin.
        ...
        # Validate that essential SHIP_FROM details are present
        required_ship_from_keys = ['name', 'street_1', 'city', 'state', 'zip', 'country', 'phone']
        missing_ship_from_keys = [key for key in required_ship_from_keys if not mock_ship_from_production.get(key)]
        if missing_ship_from_keys:
            print(f"ERROR: FedEx Smoke Test ABORTED. Essential SHIP_FROM details missing in .env: {', '.join(missing_ship_from_keys)}")
            exit()

        # Validate that essential RECIPIENT details are present (replace placeholders)
        required_recipient_keys = ['customer_shipping_address_line1', 'customer_shipping_city', 'customer_shipping_state', 'customer_shipping_zip']
        for key in required_recipient_keys:
            if 'YOUR_TEST_RECIPIENT' in mock_order_data_smoke_test.get(key, ""):
                print(f"ERROR: FedEx Smoke Test ABORTED. Please replace placeholder '{key}' in mock_order_data_smoke_test.")
                exit()


        service_to_test = "Fedex Ground" # Use the string your map_shipping_method_to_fedex_code expects
        weight_to_test = 1  # Small weight in lbs

        print(f"\nAttempting Production FedEx Label (Bill SENDER - {service_to_test})")
        print(f"Shipping FROM: {mock_ship_from_production['street_1']}, {mock_ship_from_production['city']}")
        print(f"Shipping TO:   {mock_order_data_smoke_test['customer_shipping_address_line1']}, {mock_order_data_smoke_test['customer_shipping_city']}")
        print(f"Billing TO:    Shipper Account {FEDEX_SHIPPER_ACCOUNT_NUMBER}") # This should be your production account


        # Ensure generate_fedex_label_raw is using the correct payment_info for SENDER
        # (The version you last provided with the temporary SENDER force or the restored original logic)
        # For this smoke test, the original logic where it defaults to SENDER if 'is_bill_to_customer_fedex_account' is false, is correct.
        ...
        if pdf_bytes_fx and tracking_fx:
            print(f"FedEx Production Smoke Test (Bill Sender): SUCCESS!")
            print(f"  Tracking Number: {tracking_fx}")
            print(f"  Label PDF Size: {len(pdf_bytes_fx)} bytes")
            filename = f"production_fedex_label_{tracking_fx}.pdf"
            try:
                with open(filename, "wb") as f:
                    f.write(pdf_bytes_fx)
                print(f"  Saved production label as {filename}")
                print("  ACTION: Please visually inspect the PDF label for correctness.")
                print(f"  ACTION: Check your FedEx account online for shipment {tracking_fx} and confirm billing.")
            except Exception as e_write:
                print(f"  Error writing production FedEx label PDF to file: {e_write}")
        elif tracking_fx:
            print(f"FedEx Production Smoke Test (Bill Sender): PARTIAL SUCCESS.")
            print(f"  Tracking Number: {tracking_fx}")
            print(f"  BUT no PDF label bytes were returned.")
        else:
            print(f"FedEx Production Smoke Test (Bill Sender): FAILED. No tracking or label generated.")
            print(f"  Check logs above for API error details.")

    else:
        print("\nFedEx Production Smoke Test: SKIPPED, no FedEx OAuth token obtained.")

    print("\n--- FedEx Production Smoke Test for shipping_service.py finished ---")
